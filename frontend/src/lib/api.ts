...
export async function updatePlace(token: string, placeId: string, payload: PlacePayload): Promise<PlaceRecord> {
...
export async function searchLocations(query: string): Promise<PlaceSearchResult[]> {
  const result = await apiRequest<{ results: PlaceSearchResult[] }>(`/geo/search?q=${encodeURIComponent(query)}`, null, {
    method: 'GET'
  });
