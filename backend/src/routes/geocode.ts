...
function getRegionName(address: NominatimAddress | undefined, fallback: string): string {
  return (
    address?.city ??
    address?.town ??
    address?.village ??
...
geocodeRouter.get('/reverse', async (request, response) => {
  const latitude = typeof request.query.lat === 'string' ? Number(request.query.lat) : NaN;
  const longitude = typeof request.query.lon === 'string' ? Number(request.query.lon) : NaN;
