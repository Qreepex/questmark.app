...
function buildUsernameCandidates(baseUsername: string, subject: string): string[] {
...
async function ensureUser(subject: string, preferredUsername: string) {
  ...
  const usernameCandidates = buildUsernameCandidates(preferredUsername, subject);
...
authRouter.get('/callback', async (request, response) => {
  ...
  if (!userInfo.sub) {
    response.status(502).json({ error: 'OAuth profile did not include a subject' });
    return;
  }

  const preferredUsername = userInfo.preferred_username ?? userInfo.name ?? userInfo.sub;
  ...
  response.redirect(`${getRequiredEnv('FRONTEND_URL')}/auth/callback#token=${encodeURIComponent(token)}`);
});
